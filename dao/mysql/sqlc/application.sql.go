...
const createGet = `-- name: CreateGet :one
select account1_id, account2_id, apply_msg, refuse_msg, status, create_at, update_at from applications where account1_id = ?
`

func (q *Queries) CreateGet(ctx context.Context, account1ID int64) (*Application, error) {
	row := q.queryRow(ctx, q.createGetStmt, createGet, account1ID)
	var i Application
	err := row.Scan(
		&i.Account1ID,
		&i.Account2ID,
