...
func Cors() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if origin != "" {
			...
			// 允许跨域设置可以返回其他子段，可以自定义字段
			ctx.Writer.Header().Set("Access-Control-Allow-Headers", "Access-Control-Allow-Origin,content-type,Authorization,Content-Length,X-CSRF-AccessToken,AccessToken,session")
			// 允许浏览器（客户端）可以解析的头部（重要）
